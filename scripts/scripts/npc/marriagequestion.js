...
function action(mode, type, selection) {
	if (mode == -1) {
		otherChar.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "Your partner has declined your request."));
		cm.dispose();
	} else {
		if (mode == 0 && status == 0) {
			otherChar.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "Your partner has declined your request."));
		...
		if (status == 0) {
			...
			otherChar.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(1, "Your partner has accepted your request."));
