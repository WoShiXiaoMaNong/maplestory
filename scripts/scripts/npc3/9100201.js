	...
	if (mode == -1) {
		...
		if (status == 0) {
			...
			if(finalitem.length != 0){
				...
				cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, Packages.tools.MaplePacketCreator.getItemMega(cm.getC().getChannel(),cm.getPlayer().getName() + " : " + "从卷轴抽奖机获得！大家一起恭喜他（她）吧！！！",toDrop, true).getBytes());				
				cm.gainNX(-999);
