...
//-----------------------------------------------------------------------------
	...
	if(selection == 6){
		...
		cm.c.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.updatePlayerStats(statup));
...
cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, Packages.tools.MaplePacketCreator.getItemMegas(cm.getC().getChannel(),cm.getPlayer().getName() + " : " + "从飞天猪获得！大家一起恭喜他/她吧！！！",toDrop, true).getBytes());
