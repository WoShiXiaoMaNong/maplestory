...
function startInstance() {//怪物攻城
    ...
    em.getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0,"[十万火急] 1线市场突然出现一头巨兽，是英雄就拿起宝刀K掉吧!"));
...
function started() {
    ...
    if(mobCount==0){
		if(checkstart>20){
			em.getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0,"[警告] 巨兽悄悄的消失在1线市场!"));
			checkstart=0;
		}else{
			scheduleNew2();
			em.getChannelServer().broadcastPacket(Packages.tools.MaplePacketCreator.serverNotice(0,"[十万火急] 1线市场突然出现一头巨兽，是英雄就拿起宝刀K掉吧!"));
