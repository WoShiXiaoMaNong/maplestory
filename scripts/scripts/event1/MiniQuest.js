...
function playerExit(eim, player) {
	...
	player.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, "[绯红要塞] 由于未知的力量，你被送出了绯红要塞！！"));
...
function allMonstersDead(eim) {
	...
	if (monster_number > 20) {
		...
		map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[绯红要塞] 恭喜你！！你已经击败了所有绯红BOSS！！！"));
		disbandParty();
	}
	else {
		map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[绯红要塞] 恭喜你！！击败了当前怪物，下一个怪物会在10秒后召唤。"));
...
function beginQuest(eim) {
	var map = eim.getMapInstance(803001200);
	map.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(6, "[绯红要塞] 绯红怪兽会在30秒后召唤，请你们做好最周全的准备！！！"));
