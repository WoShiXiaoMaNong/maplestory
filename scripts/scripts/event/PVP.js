...
function playerEntry(eim, player) {
    ...
    if (eim.getProperty("started").equals("0")) {
        ...
        if (eim.getPlayerCount() >= (player.isGM() ? 2 : i)) {
            ...
            eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPScore(0, false));
            eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPMode(type));
            eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPMode(12)); //PVP 1.5 EVENT!
            eim.broadcastPacket(Packages.tools.MaplePacketCreator.enablePVP(true));
            ...
            if (type == 2) {
                ...
                eim.broadcastPacket(Packages.tools.MaplePacketCreator.getCapturePosition(map));
                eim.broadcastPacket(Packages.tools.MaplePacketCreator.resetCapture());
            }
            updateScoreboard(eim, true);
        }
    } else {
        if (type != 0) {
            player.getClient().getSession().write(Packages.tools.MaplePacketCreator.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        }
        player.getClient().getSession().write(Packages.tools.MaplePacketCreator.getPVPScore(0, false));
    }
}
...
function broadcastType(eim) {
    ...
    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPType(type, c1, 1, !eim.getProperty("started").equals("0"), lvl), 0);
    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPTeam(c1), 0);

    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPType(type, c2, 2, !eim.getProperty("started").equals("0"), lvl), 1);
    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPTeam(c2), 1);
}
...
function broadcastType(eim, player) {
    ...
    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPType(type, player.getTeam() == 0 ? c1: c2, player.getTeam() + 1, !eim.getProperty("started").equals("0"), lvl), player.getTeam());
    eim.broadcastTeamPacket(Packages.tools.MaplePacketCreator.getPVPTeam(player.getTeam() == 0 ? c1: c2), player.getTeam());
}
...
function addPVPScore(eim, player, score) {
    ...
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        ...
        player.getClient().getSession().write(Packages.tools.MaplePacketCreator.getPVPIceGage(xx));
...
function playerDead(eim, player) {
    player.getClient().getSession().write(Packages.tools.MaplePacketCreator.getPVPKilled(player.getName() + " has been defeated."));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
...
function championCheck(eim) {
    ...
    if (champion != null) {
        ...
        champion.getClient().getSession().write(Packages.tools.MaplePacketCreator.showOwnChampionEffect());
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPScoreboard(c, ty));
        if (ty != 0 && ty != 2) {
            eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players.get(i).getClient().getSession().write(Packages.tools.MaplePacketCreator.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1));
        }
    }
}
...
function end(eim) {
    ...
    eim.broadcastPacket(Packages.tools.MaplePacketCreator.getPVPMode(6));
    eim.broadcastPacket(Packages.tools.MaplePacketCreator.enablePVP(false));
