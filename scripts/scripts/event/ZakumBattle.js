...
function playerDisconnected(eim,player) {
	...
	if (player.getName().equals(eim.getProperty("leader"))) {
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"本队伍队长已断开连接!"));
		}
	}
...
function monsterValue(eim,mobId) { // potentially display time of death? does not seem to work
	if (mobId == 8800002) { // 3rd body
		...
		while (iter.hasNext()) {
			var pl = iter.next();
			pl.getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"恭喜打败大怪物扎昆！"));
		}
	}
	return -1;
}
