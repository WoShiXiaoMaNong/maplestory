...
function enter(pi) {
	...
	if(!pi.haveItem(4001094)){      //判断是否有九灵龙的蛋
		pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(5, "您必须拥有九灵龙的蛋才能从这里进去!")); 
		return false;
	}
	if(nextmap.playerCount() > 0){  //判断地图内是否有人进入了
		pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(5, "里面已经有人在会见九灵龙了,请稍后进入!")); 
		return false;	
	}
