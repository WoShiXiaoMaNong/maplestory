...
function enter(pi) {
	...
	if (!pi.haveItem(4000381)) {
		...
		pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6,"The door will not open, didn't Ralph say something about White Essence?"));
		return false;
	}
...
function sendMessage(pi,message) {
	pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, message));
}